...
export function ProjectsView({
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      {/* PAGE HEADER */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-900/90 pb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-white">Projects</h1>
        ...
        <button
          ...
          className="px-4 py-2 rounded-xl bg-white hover:bg-zinc-100 text-black font-bold text-xs font-mono shadow-sm transition-all cursor-pointer self-start sm:self-auto"
        >
          + New Project
        </button>
      </div>
      ...
      {/* FILTER BAR */}
      <div className="p-3.5 rounded-2xl bg-zinc-950/80 border border-zinc-800/80 flex flex-wrap items-center justify-between gap-3 text-xs font-mono">
        <div className="relative flex-1 min-w-[200px]">
          <input
            ...
            className="w-full px-3.5 py-1.5 rounded-xl bg-black border border-zinc-800 text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-zinc-500 font-sans pl-8"
          />
      ...
      <div className="p-6 sm:p-7 rounded-2xl bg-zinc-950/80 border border-zinc-800/80 space-y-5 relative overflow-hidden group">
        <div className="flex items-center justify-between text-xs font-mono text-zinc-400">
          <span className="px-2.5 py-0.5 rounded bg-zinc-900 border border-zinc-800 text-zinc-300 font-bold">
            FEATURED CASE STUDY
          </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-center">
          <div className="lg:col-span-2 space-y-3">
            ...
            <div className="flex flex-wrap gap-1.5 font-mono text-[10px] pt-1">
              {["Next.js 16", "TypeScript", "Prisma", "PostgreSQL"].map((t) => (
                <span key={t} className="px-2 py-0.5 rounded bg-black border border-zinc-800 text-zinc-300">
                  {t}
                </span>
              ))}
            </div>
          ...
          <div className="lg:col-span-1 flex flex-col justify-between bg-black/60 p-4 rounded-xl border border-zinc-800/80 space-y-3">
            <div className="font-mono text-xs space-y-1">
              <div className="text-zinc-500 text-[10px]">Built By</div>
              <div className="text-white font-bold">{user.name}</div>
              <div className="text-zinc-400 text-[10px]">{user.university}</div>
            </div>

            <button
              onClick={() => onSelectProject && projectsList[0] && onSelectProject(projectsList[0])}
              className="w-full py-2 rounded-xl bg-white hover:bg-zinc-100 text-black font-bold text-xs font-mono text-center shadow-sm transition-all cursor-pointer"
            >
              View Case Study ➔
            </button>
          </div>
      ...
      <div className="space-y-4">
        ...
        {filteredProjects.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredProjects.map((project) => (
              ...
              >
                ...
                <div className="space-y-2.5 pt-3 border-t border-zinc-900/90 font-mono text-xs">
          ...
          <div className="p-10 rounded-2xl bg-zinc-950/80 border border-zinc-800/80 text-center space-y-3">
            <h3 className="text-lg font-bold text-white">Your portfolio starts here.</h3>
            ...
            <button
              onClick={onOpenNewProjectModal}
              className="px-4 py-2 rounded-xl bg-white hover:bg-zinc-100 text-black font-bold text-xs font-mono transition-all cursor-pointer"
            >
      ...
      <div className="space-y-4 pt-2">
        <div className="border-b border-zinc-900/90 pb-3 flex items-center justify-between">
          <div>
            <h2 className="text-base font-bold text-white tracking-tight">Discover Student Projects</h2>
            <p className="text-xs font-mono text-zinc-400">Curated showcase of software built by university developers across India.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {communityProjects.map((cp) => (
            <div
              key={cp.id}
              className="p-5 rounded-2xl bg-zinc-950/80 border border-zinc-800/80 hover:border-zinc-700 transition-all space-y-3 flex flex-col justify-between"
            >
              <div className="space-y-2.5">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="text-[11px] font-mono text-zinc-400">
                      by <span className="text-white font-bold">@{cp.creatorHandle}</span> • {cp.university}
                    </div>
                  </div>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-zinc-900/90 font-mono text-xs">
                <a
                  href={cp.github}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-3 py-1 rounded-xl bg-zinc-900 hover:bg-white hover:text-black border border-zinc-800 text-zinc-300 text-xs transition-all"
                >
                  View Project ➔
                </a>
