...
export function Sidebar({ activeNav, setActiveNav, user, projectsCount }: SidebarProps) {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("skillsphere_sidebar_collapsed");
      if (saved !== null) {
  ...
  const toggleCollapse = () => {
    setIsCollapsed((prev) => {
      const next = !prev;
      try {
        localStorage.setItem("skillsphere_sidebar_collapsed", JSON.stringify(next));
      } catch (e) {}
      return next;
    });
  };
  ...
  return (
    <>
      <aside
        ...
        } bg-[#faf6f0] border-r border-[#e8e2d8] flex flex-col justify-between hidden md:flex sticky top-0 h-screen select-none z-30 shrink-0 transition-all duration-300`}
      >
        <div className="p-4 space-y-6">
          {/* Brand Header / Logo Toggle Button */}
          <button
            onClick={toggleCollapse}
            className={`w-full flex items-center gap-2.5 px-2 py-1 rounded-xl hover:bg-[#f4efe6] transition-all group cursor-pointer ${
              isCollapsed ? "justify-center" : "justify-start"
            }`}
            title={isCollapsed ? "Expand Sidebar" : "Collapse Sidebar"}
          >
            <img src="/SSblacky.png" alt="SkillSphere Logo" className="h-7 w-auto object-contain shrink-0 group-hover:scale-105 transition-transform" />
            {!isCollapsed && (
          ...
          <nav className="space-y-1.5 font-mono text-xs">
            {navItems.map((item) => (
              <button
                ...
                className={`w-full flex items-center transition-all font-medium cursor-pointer ${
                  isCollapsed
                    ? "justify-center p-3 rounded-2xl border"
                    : "justify-between px-3 py-2.5 rounded-xl text-left"
                } ${
                  activeNav === item.id
                    ? isCollapsed
                      ? "bg-zinc-900 text-white border-zinc-900 shadow-sm"
                      : "bg-zinc-900 text-white font-extrabold shadow-xs"
                    : "text-zinc-700 hover:text-zinc-900 hover:bg-[#f4efe6] border-transparent"
                }`}
              >
